...
if(isset($_POST['Submit'])) {	
	...
	if(empty($id) || empty($monto) || empty($cuotaInicial) || empty($tasa) || empty($fechaDes) || empty($cuotas) || empty($idUsuario) || empty($fecha) || empty($cliente) || empty($observacion)) {
		...
		$result = mysqli_query($mysqli, "INSERT INTO tbl_creditos(id_cre,mon_cre,cuo_ini_cre,tas_cre,fec_des_cre,cuo_cre,id_usu_cre,fec_cre,cli_cre,obs_cre) VALUES('$id','$monto','$cuotaInicial','$tasa','$fechaDes','$cuotas','$idUsuario','$fecha','$cliente','$observacion')");
		
		echo "<font color='green'>Credito agregado correctamente.";
		echo "<br/><a href='indexCredito.php'>
