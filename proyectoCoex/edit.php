...
{	
	...
	if(empty($id) || empty($nombre) || empty($apellido) || empty($ciudad) || empty($telefono) || empty($direccion) || empty($cupoDisponible) || empty($cupoTotal) || empty($dias)) {
		...
		if(empty($ciudad)) {
			echo "<font color='red'>ciudad field is empty.</font><br/>";
		}
		if(empty($telefono)) {
		...
		if(empty($cupoDisponible)) {
			echo "<font color='red'>cupo disponible field is empty.</font><br/>";
		}
		if(empty($cupoTotal)) {
			echo "<font color='red'>cupo total field is empty.</font><br/>";
		}
		if(empty($dias)) {
			echo "<font color='red'>dias field is empty.</font><br/>";
		}	
	} else {	
		$result = mysqli_query($mysqli, "UPDATE tbl_usuarios SET nom_usu='$nombre',ape_usu='$apellido',ciu_usu='$ciudad',tel_usu='$telefono',dir_usu='$direccion',cup_dis_usu='$cupoDisponible',cup_tot_usu='$cupoTotal',dia_usu='$dias' WHERE id_usu=$id");
...
$result = mysqli_query($mysqli, "SELECT * FROM tbl_usuarios WHERE id_usu=$id");

while($res = mysqli_fetch_array($result))
{
	$id = $res['id_usu'];
	$nombre = $res['nom_usu'];
	$apellido = $res['ape_usu'];
	$ciudad = $res['ciu_usu'];
	$telefono = $res['tel_usu'];
	$direccion = $res['dir_usu'];
	$cupoDisponible = $res['cup_dis_usu'];
	$cupoTotal = $res['cup_tot_usu'];
	$dias = $res['dia_usu'];
}
?>
<html>
...
<main class="container row">
        ...
        <div class="contenedor2 col-9" >
			...
			<div>
			<a href="index.php" class="fs-5 "><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-box-arrow-left" viewBox="0 0 16 16">
  <path fill-rule="evenodd" d="M6 12.5a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-8a.5.5 0 0 0-.5.5v2a.5.5 0 0 1-1 0v-2A1.5 1.5 0 0 1 6.5 2h8A1.5 1.5 0 0 1 16 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-8A1.5 1.5 0 0 1 5 12.5v-2a.5.5 0 0 1 1 0v2z"/>
  <path fill-rule="evenodd" d="M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3z"/>
</svg> Atras</a>
	<br/><br/>
	
	<form name="form1" method="post" action="edit.php" class="p-3">
		<table >
			<tr> 
				<td>nit</td>
				<td><input type="text" name="id_usu" value="<?php echo $id;?>" ></td>
			</tr>
			<tr> 
				<td>nombre</td>
				<td><input type="text" name="nom_usu" value="<?php echo $nombre;?>"></td>
			</tr>
			<tr> 
				<td>apellido</td>
				<td><input type="text" name="ape_usu" value="<?php echo $apellido;?>"></td>
			</tr>
			<tr> 
				<td>ciudad</td>
				<td><input type="text" name="ciu_usu" value="<?php echo $ciudad;?>"></td>
			</tr>
			<tr> 
				<td>telefono</td>
				<td><input type="text" name="tel_usu" value="<?php echo $telefono;?>"></td>
			</tr>
			<tr> 
				<td>direccion</td>
				<td><input type="text" name="dir_usu" value="<?php echo $direccion;?>"></td>
			</tr>
			<tr> 
				<td>cupo disponible</td>
				<td><input type="text" name="cup_dis_usu" value="<?php echo $cupoDisponible;?>"></td>
			</tr>
			<tr> 
				<td>cupo total</td>
				<td><input type="text" name="cup_tot_usu" value="<?php echo $cupoTotal;?>"></td>
			</tr>
			<tr> 
				<td>dias</td>
				<td><input type="text" name="dia_usu" value="<?php echo $dias;?>"></td>
			</tr>
			<tr>
